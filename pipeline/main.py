...
def data_aggregation(data):
    try:
        ...
        data_aggregated['Sum_price_per_customer']=data.groupby(['customer_id','product_id'])['price'].sum()
        data_aggregated['Mean_quantity_per_product']=data.groupby(['customer_id','product_id'])['quantity'].mean()
        data_aggregated['Max_product_quantity']=data.groupby('product_id')['quantity'].sum().idxmax()
        data_aggregated['Max_product_price']=data.groupby('product_id')['price'].sum().idxmax()
        data_aggregated['Max_month_per_quantity']=data.groupby('month')['quantity'].sum().idxmax()
        data_aggregated['Max_month_per_price']=data.groupby('month')['price'].sum().idxmax()
        data_aggregated['Max_quarter_per_quantity']=data.groupby('quarter')['quantity'].sum().idxmax()
        data_aggregated['Max_quarter_per_price']=data.groupby('quarter')['price'].sum().idxmax()  
        write_db(data_aggregated,'Data_Aggregation')
    except:
        conn = sqlite3.connect('DataPipeline')
...
def main():
    try:
        WEATHER_API='https://api.openweathermap.org/data/2.5/weather?appid=a8ce3805722343fe8d97a1cc8a693881&lat='
        CUSTOMERS_API='https://jsonplaceholder.typicode.com/users'
        user_data=data_extraction(CUSTOMERS_API)
        ...
        for i in range (len(user_data)):
            if user_data['customer_id'][i] not in weather:                          
                weather[user_data['customer_id'][i]]=data_extraction(WEATHER_API+
                                                                     user_data['address.geo.lat'][i]+
                                                                     '&lon='
                                                                     +user_data['address.geo.lng'][i])
                
                df_weather=df_weather._append(pd.DataFrame({'customer_id':user_data['customer_id'][i],
                                                            'temp':weather[user_data['customer_id'][i]]['main.temp'],
                                                            'weather':weather[user_data['customer_id'][i]]['weather'][0]}))
                
        merged_data=pd.merge(sales_data,user_data,
                             on='customer_id')
        
        merged_data=pd.merge(merged_data,df_weather,
                             on='customer_id')
